...
12. Public repo: gitignore-class artifacts and PII must be purged from git *history*, not just HEAD. When ignoring something ever tracked: find it with `git log --all --diff-filter=A --name-only`, scrub (filter-branch/filter-repo with `--tag-name-filter cat`), then `reflog expire --expire=now --all` + `gc --prune=now`. `git bundle` backup first; remap commit SHAs referenced in docs. Force-pushing rewritten history: only the user runs it, explicitly.
