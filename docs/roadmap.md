...
- **Legal audit (`/legal`).** Deferred to this phase: inventory every dependency and bundled/auto-downloaded model-weight license (Real-ESRGAN, GFPGAN, video2x, Python deps), confirm each permits redistribution, write `THIRD_PARTY_NOTICES`, and add the AS-IS / no-warranty / no-liability disclaimer. Feeds the `/ship` legal stage below.
